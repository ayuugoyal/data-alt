...
CUSTOM_DOMAIN=""  # Will be set if user provides custom domain
...
get_cloudflare_info() {
    ...
    echo
    read -p "Do you want to use a custom domain/subdomain? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        read -p "Enter your custom domain/subdomain (e.g., sensor.mydomain.com): " CUSTOM_DOMAIN
        if [ -n "$CUSTOM_DOMAIN" ]; then
            print_status "Will configure tunnel for: $CUSTOM_DOMAIN"
            echo
            print_warning "Make sure your domain is added to Cloudflare and DNS is managed by Cloudflare!"
            echo "1. Add your domain to Cloudflare Dashboard"
            echo "2. Update nameservers at your domain registrar (Hostinger)"
            echo "3. Wait for DNS propagation (24-48 hours)"
            echo
            read -p "Press Enter when your domain is ready in Cloudflare..."
        fi
    fi
}
...
create_cloudflare_tunnel() {
    ...
    if [ -n "$CUSTOM_DOMAIN" ]; then
        ...
        if cloudflared tunnel route dns "$TUNNEL_NAME" "$CUSTOM_DOMAIN"; then
            ...
            print_status "You may need to create it manually in Cloudflare Dashboard"
            print_status "Create a CNAME record: $CUSTOM_DOMAIN → $TUNNEL_ID.cfargotunnel.com"
        fi
    else
...
EOF
    ...
    print_success "Tunnel configuration created for $HOSTNAME"
}
...
if [ \$? -eq 0 ] && echo "\$TUNNEL_LIST" | grep -q "$TUNNEL_NAME"; then
    ...
    echo "   https://\$HOSTNAME/ (Homepage)"
    echo "   https://\$HOSTNAME/docs (Swagger UI)"
    echo "   https://\$HOSTNAME/sensors (All sensors)"
    echo "   https://\$HOSTNAME/sensors/ultrasonic (Distance)"
    echo "   https://\$HOSTNAME/sensors/mq135 (Air quality)"
    echo "   https://\$HOSTNAME/sensors/dht11 (Temperature/Humidity)"
    echo "   https://\$HOSTNAME/sensors/alerts (Alerts)"
    echo "   https://\$HOSTNAME/health (Health check)"
    echo
...
display_usage_info() {
    ...
    if [ -f "$PROJECT_DIR/.tunnel_hostname" ]; then
        FINAL_URL=$(cat "$PROJECT_DIR/.tunnel_hostname")
    else
        FINAL_URL="$TUNNEL_NAME.cfargotunnel.com"
    fi
    ...
    echo "🌐 External URL: https://$FINAL_URL"
    echo
    ...
    echo "  Remote: curl https://$FINAL_URL/sensors"
    echo
    echo "🌐 Your External URL: https://$FINAL_URL"
